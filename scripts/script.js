...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const templateProductRow = document.getElementById(
    "template-product-row"
  ).content;

  const productRowTotalTable = document.querySelector(
    ".product-row-total-table"
  );
  const templateTotalTable = document.getElementById(
    "template-total-table"
  ).content;
  ...
  const hiddenMenu = () => {
    ...
    document
      ...
      .setAttribute("src", "images/cart_empty_blue.png");
  ...
  document
    .querySelector(".offcanvas-backdrop")
    .addEventListener("click", toggleCartDisplay);
  document
    .querySelector(".fa-arrow-up-from-bracket")
    .addEventListener("click", toggleCartDisplay);
  ...
  document.querySelector(".fa-bars").addEventListener("click", () => {
    if (
      document
        .querySelector(".cart-container")
        .classList.contains("show-cart-container")
    )
      toggleCartDisplay();
    ...
    if (document.querySelector(".navbar").classList.contains("navbar-show"))
      document
        .querySelector(".cart-img")
        .setAttribute("src", "images/cart_empty_white.png");
    else
      document
        .querySelector(".cart-img")
        .setAttribute("src", "images/cart_empty_blue.png");
  ...
  const getProductAPI = () => {
    try {
      fetch("https://jsonblob.com/api/1061116324926996480").then((result) => {
        result.json().then((datos) => {
          ...
          productRow.addEventListener("click", (e) => {
            ...
            if (e.target.classList.contains("inc")) {
              cart.actualizarUnidades(
                sku,
                cart.obtenerInformacionProducto(sku).quantity + 1
              );
              update(cart);
            }
            ...
            if (e.target.classList.contains("fa-trash-can")) {
              ...
              if (cart.obtenerCarrito().products.length === 0) {
                ...
                productRow.innerHTML =
                  '<span class="text-cart-empty">Todos los productos en el carrito han sido eliminados, por favor inserte nuevos productos</span>';
              } else update(cart);
            }
          });
          ...
          document
            .querySelector(".producto-description")
          ...
          document
            ...
            .addEventListener("click", (e) => {
              if (e.target.classList.contains("add-cart-btn")) {
                ...
                card
                  .querySelector(".succefull")
                  .classList.toggle("show-succefull");
                window.setTimeout(() => {
                  card
                    .querySelector(".succefull")
                    .classList.toggle("show-succefull");
      ...
      alert(
        "El sitio lanzó el siguiente error: " + error + ", al parecer hubo algún problema a la hora de cargar la API"
      );
    }
  };
  ...
  const update = (cart) => {
    document.querySelector(".full").textContent =
      cart.obtenerCarrito().products.length;
    printCart(cart.obtenerCarrito());
  };
  ...
  const printCart = (cart) => {
    ...
    productList.forEach((elem) => {
      ...
      templateProductRow.querySelector("#product-cost").textContent =
        Math.round(elem.price * elem.quantity * 100) / 100 + cart.currency;
  ...
  const printTotalTable = (cart) => {
    ...
    document.querySelector(".last-ul span").textContent =
      Math.round(cart.total * 100) / 100 + cart.currency;
    const productList = cart.products;
    productList.forEach((elem) => {
      if (elem.quantity !== 0) {
        templateTotalTable.querySelectorAll("li")[0].textContent = elem.title;
        templateTotalTable.querySelector("span").textContent =
          Math.round(elem.price * elem.quantity * 100) / 100 + cart.currency;
